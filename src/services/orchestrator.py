...
class Orchestrator:
    ...
    async def process_single_client(self, client):
        ...
        if cashed:
            person_info = client[self.contact_service.fields.info_fedresurs] # берем данные из кэша
    ...
    async def handle_matched_person(self, client, person, cached = False):
        if cached:
            messages = client[self.contact_service.fields.messages_fedresurs]
        else:
            messages = await self.fedresurs_service.get_messages(person["id"])

        case_num = "" # номер дела о банкротстве (по умолчанию нет)
        for message in messages:
            case_num = await self.process_message(client, person, message) # в некоторых сообщениях он есть, по этому мы берем его из них
    ...
    async def process_message(self, client, person, message, cashed: bool = False) -> str:
        ...
        if cashed:
            ...
            message_info = message_bitrix[self.message_service.fields.cache]
        else:
    ...
    async def get_persons_for_client(self, client):
        ...
        if valid := self.is_cache_valid(client):
            return valid, [client[self.contact_service.fields.find_fedresurs]]
